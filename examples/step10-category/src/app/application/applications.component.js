	...
	app.ApplicationsComponent = ng.core
		...
		.Class({
			...
			ngOnInit: function(){
				...
				var params=this.activatedRoute.params.value; // 路由参数
				if(params.category){
					// 查询分类对应的应用					
						this.service.getCategories().then(categories=>{this.categories = categories});
						this.service.getApplicationsByCategory(params.category).then(applications => this.applications = applications);
...
//				this.applications = this.service.getApplications();
			...
			constructor: [ng.router.ActivatedRoute, ng.router.Router, app.ApplicationService, function(activatedRoute, router, service) {
				// 保存 ActivatedRoute 对象
